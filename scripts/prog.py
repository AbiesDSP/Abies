...
if __name__ == '__main__':
    ...
    if ('Microsoft' in uname().release):
        ...
        if (len(sys.argv) == 3 and sys.argv[2] == '--flash'):
            ...
            # Program flash.
            prog_flash_cmd = f'{vloc}\\{vver}\\settings64.bat && cd {project_name}-viv && vivado -notrace -mode batch -source ../scripts/program_flash.tcl -tclargs {bit_file} {flash_file} {prm_file} {mem_part}'
            viv_win_cmd = ["/mnt/c/Windows/System32/cmd.exe", "/c", prog_flash_cmd]
            subprocess.run(viv_win_cmd)

        else:
            # Program temporary
            win_prog_cmd = f'{vloc}\\{vver}\\settings64.bat && cd {project_name}-viv && vivado -notrace -mode batch -source scripts/program.tcl -tclargs {bit_file} && cd ..'
