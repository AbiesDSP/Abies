...
def new(args):
    ...
    if args.project != None:
        new_project(args, template_filter)
    else:
        new_module(args, template_filter)
...
def new_project(args, template_filter):
...
def new_module(args, template_filter):

    # only create a module if nothing exists already.
    base_project = os.path.join(os.getcwd(), args.module)
    if os.path.exists(base_project):
        print("Module: '" + args.module + "' already exists!")
        return
    # Create new directory, or reused existing directory.
    base_project = create_new_dir(args.module)
    os.makedirs(os.path.join(base_project, 'tests'))
    ...
    with pkg_resources.open_text(resources, "test_module.cpp") as file:
       apply_template(file, 'test_' + args.module + '.cpp', os.path.join(base_project, 'tests'), template_filter)
